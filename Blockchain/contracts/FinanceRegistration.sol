...
contract FinanceRegistration {
    ...
    function registerSME(string memory _businessID, string memory _companyName, string memory _contactDetails) public {
        ...
        // account for the SME
        address smeAddress = address(uint160(uint(keccak256(abi.encodePacked(block.timestamp, msg.sender))))); // Pseudo-random generation of address
        // bytes memory privateKey = abi.encodePacked(block.timestamp, msg.sender);
    ...
    function getSMEData(string memory _businessID) public view returns (string memory, string memory, string memory, address) {
        SME memory sme = smeRegistry[keccak256(bytes(_businessID))];
        require(bytes(sme.businessID).length > 0, "SME not found");
        return (sme.businessID, sme.companyName, sme.contactDetails, sme.smeAddress);
    }
